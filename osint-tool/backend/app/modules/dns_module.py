...
Every lookup goes through `core.resolver`, which reuses a single resolver
instance (this module issues ~15 queries per scan) and falls back to
DNS-over-HTTPS when the local resolver cannot answer.
"""
...
from ..core.resolver import resolve, resolve_txt, reverse
...
class DNSModule(OSINTModule):
    ...
    async def _query_record(self, domain: str, rtype: str) -> list[Finding]:
        ...
        for raw in await resolve(domain, rtype):
            val = raw.strip('"')
    ...
    async def _query_email_security(self, domain: str) -> list[Finding]:
        ...
        checks = [
            (f"_mta-sts.{domain}", "v=stsv1", "MTA-STS", "Enabled (enforced TLS policy)"),
            (f"_smtp._tls.{domain}", "v=tlsrptv1", "TLS-RPT", "Enabled (TLS reporting)"),
            (f"default._bimi.{domain}", "v=bimi1", "BIMI", "Enabled (verified brand logo)"),
        ]
        records = await asyncio.gather(*(resolve_txt(name) for name, *_ in checks))
        findings: list[Finding] = []
        for (_, needle, label, value), values in zip(checks, records, strict=True):
            if any(v.lower().startswith(needle) for v in values):
                findings.append(Finding(label=label, value=value, category="email_security", confidence=0.9))
        return findings

    async def _query_dnssec(self, domain: str) -> list[Finding]:
        """DNSKEY presence indicates the zone is DNSSEC-signed."""
        if await resolve(domain, "DNSKEY"):
            return [
                Finding(
                    label="DNSSEC",
                    value="Signed zone (DNSKEY present)",
                    category="dns",
                    confidence=0.9,
                )
            ]
    ...
    async def _query_www(self, domain: str) -> list[Finding]:
        ...
        cnames = await resolve(f"www.{domain}", "CNAME")
        if cnames:
            # A CNAME precludes an A record at the same name.
            return [Finding(label="www (CNAME)", value=v.rstrip("."), category="dns") for v in cnames]
        return [
            Finding(label="www (A)", value=v.rstrip("."), category="dns")
            for v in await resolve(f"www.{domain}", "A")
        ]

    async def _query_dmarc(self, domain: str) -> list[Finding]:
        """DMARC policy lives in _dmarc.<domain> as a TXT record."""
        for val in await resolve_txt(f"_dmarc.{domain}"):
    ...
    async def _reverse_dns(self, domain: str) -> list[Finding]:
        ...
        a, aaaa = await asyncio.gather(resolve(domain, "A"), resolve(domain, "AAAA"))
        ips = a + aaaa
        ptrs = await asyncio.gather(*(reverse(ip) for ip in ips))
        return [
            Finding(label=f"Reverse DNS ({ip})", value=name, category="dns")
            for ip, names in zip(ips, ptrs, strict=True)
            for name in names
        ]
